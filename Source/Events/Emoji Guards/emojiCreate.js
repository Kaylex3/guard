...
module.exports = async (emoji) => {
  ...
  try {
    const embed = new MessageEmbed()
      ...
      .setDescription(`> ${entry.executor} -> Bir emoji açıldı ve gerekli işlemler uygulandı!!`)
      .addField("Yetkili ↷", "```" + `${entry.executor.tag} | ${entry.executor.id}` + "```")
      .addField("Emoji ↷", "```" + `${emoji.name} | ${emoji.id}` + "```")
    ...
    log.send({ username: `${entry.executor.username} | Emoji Oluşturma Koruması`, avatarURL: `${entry.executor.avatarURL() || bot.user.avatarURL()}`, embeds: [embed] });
  } catch (err) {}
